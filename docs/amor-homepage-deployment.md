...
The homepage supports both the current local application and the older Hostinger installation. Application links appear when `parent.application.create` exists; otherwise visitors are directed to the contact section. Legal links appear only for installed legal routes, without depending on a separate legal partial. Authentication falls back to the existing `web` guard when no role guards are configured.

1. Back up the current deployed `welcome.blade.php`. On 2026-09-15, Hostinger File Manager was verified to open at the private Laravel root, with `public_html` a symlink to `public/`. The original deployed view was copied to `resources/views/amor-backup-20260915/welcome.blade.php` and its presence verified before replacement.
...
4. Verify the deployed page. Laravel normally recompiles a Blade template after its modification time changes. If compiled views remain stale and shell access is available, run `php artisan view:cache` with the same PHP executable used by the site. Laravel documents its `public/index.php` entry point and view caching in the [deployment guide](https://laravel.com/framework/docs/13.x/deployment).
5. Open the live homepage in a private window. Confirm all photos and styles load over HTTPS, then check login, the application link or contact fallback, available privacy/terms links and mobile menu. Check the active session's dashboard and logout through the existing workflow when appropriate.

The Hostinger Git page showed no connected repository on 2026-09-15. Pushing GitHub `main` does not automatically update this installation. Homepage deployment therefore uses File Manager: extract public assets into `public/amor-village`, extract the two new partials into `resources/views/partials`, and upload `welcome.blade.php` into `resources/views` last. The staging ZIPs belong in the private root. Do not connect a repository or deploy the rest of the newer local application as part of this homepage-only update.
...
Additional homepage authentication and legacy-deployment tests passed (11 tests, 97 assertions), covering guests, each supported guard, preferred sessions and fallback behavior without creating database records. Together with the earlier checks, 16 relevant tests and 150 assertions passed. PHP formatting and whitespace checks passed. After recovering the browser connection, 200% text at 320px showed no horizontal page overflow; high contrast produced black text on white surfaces; reset restored the default size; the statement link opened the disclosure and focused its summary; and Escape closed the accessibility dialog while leaving an open mobile menu intact. These are development checks, not a claim of audited accessibility conformance. Production deployment completed on 2026-09-15 through Hostinger File Manager. The homepage and all 13 public assets returned HTTP 200 over HTTPS, and the old hero text was absent. The login page returned HTTP 200 with its password form. Desktop and 390px mobile checks confirmed the new design; mobile navigation opened and closed correctly, the accessibility dialog opened and closed, all internal anchors resolved, and mobile page width had no horizontal overflow. Existing production accounts were not used for authenticated workflow tests. No manual compiled-view or hosting-cache purge was needed.
